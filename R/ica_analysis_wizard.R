...
ica_analysis_wizard <- function(outputobjectname = NULL){
  ...
  writeLines(ncomp, con = stdout(), sep = "\n", useBytes = FALSE) 
  ...
  while (answer == "Yes"){
    ...
    if(plottype %in% c("Score","Score_Ellipse","Var")){
      ...
      writeLines(CompX, con = stdout(), sep = "\n", useBytes = FALSE) 
      ...
      while(CompY == CompX) {
        ...
        texts_wizard(dog)
        CompY <- as.numeric(readline())
        writeLines(CompY, con = stdout(), sep = "\n", useBytes = FALSE) 
    ...
    if(plottype =="Topscoring"){
      ...
      texts_wizard("Choose the mode to compute abs for absolute value and posneg for positive and negative loading analyzed separately")
      opts <- c("abs","posneg")
