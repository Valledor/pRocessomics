...
spls_analysis_wizard <- function(outputobjectname=NULL){
  ...
  texts_wizard("\n***Currently pRocessomics only allows SPLS models of two components***")
  ...
  texts_wizard("\nWe have detected the following datasets available. Please enter the number of the dataset do you want to choose to perform the SPLS analysis and press [enter].")
  ...
  datasetnames<-names(datalist)
  texts_wizard("\nWe have detected the following levels in your data. Please enter the number(s) of the level(s) you want to use as predictors and press [enter].\n")
  datasetnames <- c(datasetnames)
  omiclevelX <- utils::select.list(datasetnames, preselect = NULL, multiple = T, title = NULL, graphics = FALSE)
  datasetnames <- datasetnames[!(datasetnames %in% omiclevelX)]
  if(length(datasetnames)<1) stop("FATAL ERROR: You must keep at least one omic level for using it as predict matrix")

  texts_wizard("\nPlease enter the number(s) of the level(s) you want to use as responses and press [enter].\n")
  omiclevelY <- utils::select.list(datasetnames, preselect = NULL, multiple = T, title = NULL, graphics = FALSE)
  if(length(omiclevelY)<1) stop("FATAL ERROR: You must keep at least one omic level for using it as response matrix")
  ...
  if(!is.na(treatment2col)){
    texts_wizard("\nPlease select which treatment you want to use as factor (not used for SPLS analysis, but needed for colouring plots).\n")
  ...
  ####Autotune, performance, and spls_analysis ----
  texts_wizard("\nSPLS model allows to define the number of variables to be kept after analysis. Depending on the number of retained variables the model will have different performances (see 'https://github.com/Valledor/pRocessomics/wiki/Multivariate'). Do you want to autotune the model (yes) or employ all variables (no)?")
  ...
  spls_results <- spls_analysis(datalist, annotation, initialrow, initialcolumn, treatment1col, treatment2col, treatment,omiclevelX,omiclevelY, keepX=NULL,keepY=NULL, autotune, performance, folds=3)
  texts_wizard("Press [enter] to continue")
  invisible(readline())
  
  #### Naming outputs ####
  if(is.null(outputobjectname)==TRUE) {
    outputobjectname <- paste(datalists,"_spls",sep="")
    dog<-as.character(paste("\n\nOutput object name was not set by user. Employing default name: ",paste(datalists,"_spls",sep="")))
    texts_wizard(dog)
  ...
  if(is.character(outputobjectname)!=TRUE) {
    outputobjectname <- paste(datalists,"_spls",sep="")
    dog<-as.character(paste("\n\nOutput object name was wrong. Employing default name: ",paste(tempdatasetname,"_spls",sep="")))
    texts_wizard(dog)
    if(exists(outputobjectname)==T){
      texts_wizard("\nnotice that there is an object in global environment with the same name and **it will be replaced**")
      texts_wizard("\nDo you want to change this name and provide a unique identifier?")
  ...
  texts_wizard("\nDo you want to save sPLS results in .xlsx?")
  answer <- utils::select.list(choices = truefalse)
  if(answer == "Yes"){
    texts_wizard("Saving...")
    diahora <-gsub(":","",gsub(" ","_", gsub("-","",Sys.time())))
    diahora <-substr(diahora,3,nchar(diahora)-2)
    tablefilename<-paste(diahora,"_spls_results.xlsx",sep="")
    export_table(result_list = spls_results, filename = tablefilename)
    texts_wizard(paste("\nResults saved in your working directory,",paste(getwd(),", as",sep=""), tablefilename,".", sep=" "))
  }
  
  
  #### Guided plotting ####
  texts_wizard("\nAre your ready to plot SPLS results?")
  ...
  while (answer == "Yes"){
    ...
    #plottype
    splsplotstype<-c("Distance","Distance_Ellipse", "Var", "Topscoring",  "Network","Composite")
    plottype <- utils::select.list(choices = splsplotstype)
    #Components
    CompX=1
    CompY=2
    cutoff=0
    ...
    #### Call spls_plot ####
    plot.object<-spls_plot(spls_list = spls_results,treatment = treatment,plottype = plottype,fortopscoring = fortopscoring, useannot = zzz, cutoff = cutoff, confidence = confidence)
    print(plot.object)
    
    #### Exporting spls_plot object ####  
    ...
    if(ansplot=="Yes"){
      ...
      diahora <-gsub(":","",gsub(" ","_", gsub("-","",Sys.time())))
      diahora <-substr(diahora,3,nchar(diahora)-2)
      plotfilename<-paste(diahora,"_spls_",plottype,"_plot.pdf",sep="")
      export_plot(plot.object, plotfilename, cutoff = cutoff)
      
      texts_wizard(paste("\nPlot saved in your working directory,",paste(getwd(),", as",sep=""),plotfilename, sep=" "))
    }
    texts_wizard("\nDo you want to try any other SPLS plot type?")
    answer <- utils::select.list(choices = truefalse)
  }
  
  
  #### Exporting session log ####
  texts_wizard("\nspls_analysis_Wizard: Duty Accomplished.")
