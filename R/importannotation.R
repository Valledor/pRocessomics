...
#' @usage annotation_import()
#' @details This function creates a dataframe containing a short description of the variables
...
annotation_import <- function() {
  #### Start log file ####
  diahora <-gsub(":","",gsub(" ","_", gsub("-","",Sys.time())))
  diahora <-substr(diahora,3,nchar(diahora)-2)
  #LL: diahora <- base::format(base::Sys.time(), "%y%m%d_%H%M")
  ...
  writeLines(typeofannotation, con = stdout(), sep = "\n", useBytes = FALSE) 

  texts_wizard("\n\nPlease select file with annotations (xls, xlsx, or txt).\n\n")
  readline(prompt="Press [enter] to open the window (Please note that it can be opened behind this window)")
  annotfilename <- file.choose()
  writeLines(annotfilename, con = stdout(), sep = "\n", useBytes = FALSE) 
 ...
 #LL: esto es imposible que pase, como mucho temas de permisos
  ...
  if (typeofannotation == "MapMan") {
    ...
    if (fileextension %in% c("XLS", "XLSX")) {
      ...
      if (length(sheetnames) > 1) {
        ...
        writeLines(sheetselection, con = stdout(), sep = "\n", useBytes = FALSE) 
  ...
  if (typeofannotation == "Custom") {
    ...
    texts_wizard("\ta: Variable identifiers that match exactly with dataset identifiers (character);\n")
    texts_wizard("\tb: Variable descriptions (character);\n ")
    texts_wizard("\tc: bin/group/pahtway codes (numeric);\n")
    texts_wizard("\td: bin/group/pathway descriptions (character)\n")
    ...
    if (fileextension == "TXT") {
      ...
      if (colnames(annottable) != c("BINCODE", "NAME", "IDENTIFIER", "DESCRIPTION", "TYPE") &&
        ...
        writeLines(identifier, con = stdout(), sep = "\n", useBytes = FALSE) 
        ...
        writeLines(description, con = stdout(), sep = "\n", useBytes = FALSE) 
        ...
        writeLines(bincode, con = stdout(), sep = "\n", useBytes = FALSE) 
        ...
        writeLines(name, con = stdout(), sep = "\n", useBytes = FALSE) 
  ...
  if (length(datalists) == 0) {
    ...
    if(answer == "Yes"){
      ...
      filename<-readline()
      writeLines(filename, con = stdout(), sep = "\n", useBytes = FALSE) 
  ...
  if (length(dupeIds) != 0) {
    ...
    writeLines(manual, con = stdout(), sep = "\n", useBytes = FALSE) 
