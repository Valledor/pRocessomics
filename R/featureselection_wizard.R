...
#' @title featureselection_wizard() a function for keeping more significant/less noisy variables
...
#' @details The objective of \code{featureselection_wizard} is removing those variables that do (not) 
...
featureselection_wizard<-function(){
  ...
  writeLines(featuresel.choose, con = stdout(), sep = "\n", useBytes = FALSE) 
  ...
  writeLines(datalistselection, con = stdout(), sep = "\n", useBytes = FALSE) 
  ...
  if(length(datalistselection)!=length(names(datalist))){
    texts_wizard("\n\nYou have decided not filtering all of your omic levels. Unselected layers will remain unmodified.")
    readline("\nPress [enter] to continue:")  
  }
  ...
  if(featureselected[i]=="IQR"){
    ...
    writeLines(threshold, con = stdout(), sep = "\n", useBytes = FALSE) 
    ...
    writeLines(abovebelow, con = stdout(), sep = "\n", useBytes = FALSE) 
    ...
    if(featureselected[i]=="Stat"){
      ...
      writeLines(parametric, con = stdout(), sep = "\n", useBytes = FALSE) 
      statchoice<-c("p","q")
      ...
      writeLines(stat, con = stdout(), sep = "\n", useBytes = FALSE) 
  ...
  filtereddata<-featureselection(datalist=finaldatalist, initialrow=initialrow, initialcolumn=initialcolumn, 
                                      ...
                                      method = featureselected[i], threshold = threshold, parametric=parametric, 
                                      stat = stat, abovebelow=abovebelow)
  ...
  if(answer == "Yes"){
    ...
    thename<-readline()
    writeLines(thename, con = stdout(), sep = "\n", useBytes = FALSE) 
