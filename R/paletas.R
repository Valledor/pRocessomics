...
#' \item "fontsizes"
#' }
#' @param newcustomvalues character vector specifying desired values
...
set_custom_palettes <- function(palette_type,newcustomvalues){
  ...
  if(palette_type =="treatment") {
    new_treatment_palette <- newcustomvalues
    class(new_treatment_palette) <- c("color.pRo", "treatment")
    return(new_treatment_palette)}
...
select_palette <- function(){
  ...
  choicetreat <- utils::select.list(choices = palettes,multiple = F,preselect = "Current",
  ...
  # Selecting palette for omics levels
  ...
  if(choicelvl=="Current") new_paleta_niveles <- Current
